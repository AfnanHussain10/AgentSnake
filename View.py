...
class SnakeViewer:
	...
	"""
	def __init__(self, agent, state, SPEED = 60, UnitSize = 10):
	...
	def CreateScoreBoard(self, color):
		...
		"""
		x_offset = 0.05
		return self.canvas.create_text(x_offset * self.CANVAS_WIDTH, 0.01 * self.CANVAS_HEIGHT, text=(f'{self.agentType} Score : ' + str(0)), anchor=tkinter.NW, font=("Times", 12, 'bold'), fill = color)
	...
	def UpdateScore(self):
		self.canvas.itemconfig(self.ScoreBoard, text=f'{self.agentType} Score : ' + str(self.state.snake.score))
